...
DEFAULT_PARAMS: Dict[str, float] = {
    # v2: GT shows only 26% settlement survival. Much harsher winters + fast forest reclaim.
    "winter_severity": 0.55,       # Much higher → more starvation deaths (was 0.28)
    "faction_aggression": 0.30,    # Higher → more raid deaths (was 0.18)
    "trade_activity": 0.5,
    "forest_growth_rate": 0.30,    # Much higher → fast ruin/dead-settlement→forest (was 0.12)
    "expansion_rate": 0.25,        # Lower → fewer new settlements (was 0.40)
    "raid_range": 5.0,
    "food_per_forest": 0.35,       # Lower → less forest food buffer (was 0.45)
    "port_development_threshold": 0.5,
    "ruin_reclaim_rate": 0.10,     # Lower → fewer ruin reclaims (was 0.20)
}
...
class NorseSimulator:
    ...
    def _phase_winter(
        ...
        for s in settlements:
            ...
            # Food loss from winter — 2-3x harsher than v1 (GT: only 26% survive)
            food_loss = severity * (1.5 + 1.5 * rng.random()) * (1.0 + s.population * 0.25)
            s.food -= food_loss

            # Population attrition in harsh winters — more aggressive
            if s.food < 0:
                pop_loss = min(abs(s.food) * 0.6, s.population * 0.6)
                s.population -= pop_loss
                s.food = 0.0

            # Natural population decay (disease, old age, emigration) — harsher
            s.population *= (0.93 + 0.04 * rng.random())

            # Settlement collapse check — much more aggressive
            if s.population < 0.5 or (s.food < 0.3 and rng.random() < 0.5 * severity):
                s.alive = False
                # Dead settlements: 50% become forest (reclaimed by nature),
                # 35% become plains, 15% leave ruins
                adj_forest = self._count_forest_food(grid, s.y, s.x) / max(self.params["food_per_forest"], 0.1)
                roll = rng.random()
                forest_chance = 0.50 + 0.10 * min(adj_forest, 3)  # More forest nearby = more likely
    ...
    def _phase_environment(
        ...
        for ry, rx in zip(ruin_ys, ruin_xs):
            ...
            if not reclaimed:
                ...
                # Age factor: young ruins exit very fast (5x higher than v1)
                # Ruins should become forest within 10-20 years
                age_factor = max(0.5, 3.0 / (1.0 + ruin_age * 0.2))

                # Forest reclamation — 5x faster than v1
                if adj_forest > 0:
                    forest_prob = age_factor * forest_rate * 0.75 * min(adj_forest, 3)
                else:
                    forest_prob = age_factor * forest_rate * 0.40

                # Plains fade — secondary path
                empty_prob = age_factor * 0.03
    ...
    def run_monte_carlo(
        ...
        # Floor at 0.0005 (not 0.01) — let near-impossible stay near-zero
        probs = np.maximum(probs, 5e-4)
        probs /= probs.sum(axis=2, keepdims=True)

        # Force mountain cells to deterministic [0,0,0,0,0,1]
        initial = np.asarray(initial_grid, dtype=np.int32)
        mountain_mask = initial == MOUNTAIN
        if mountain_mask.any():
            probs[mountain_mask] = 5e-4
            probs[mountain_mask, 5] = 1.0
            probs[mountain_mask] /= probs[mountain_mask].sum(axis=1, keepdims=True)

        # Force ocean cells to deterministic [1,0,0,0,0,0]
        ocean_mask = initial == OCEAN
        if ocean_mask.any():
            probs[ocean_mask] = 5e-4
            probs[ocean_mask, 0] = 1.0
            probs[ocean_mask] /= probs[ocean_mask].sum(axis=1, keepdims=True)
