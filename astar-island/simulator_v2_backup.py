...
DEFAULT_PARAMS: Dict[str, float] = {
    # v2: GT shows 26% settlement survival. Harsher winters + faster forest reclaim.
    "winter_severity": 0.40,       # Harsher (was 0.28)
    "faction_aggression": 0.25,    # More raids (was 0.18)
    "trade_activity": 0.5,
    "forest_growth_rate": 0.10,    # Controlled regrowth (was 0.12)
    "expansion_rate": 0.70,        # Very high expansion to match GT Empty->Sett=9%
    "raid_range": 5.0,
    "food_per_forest": 0.40,       # Slightly less (was 0.45)
    "port_development_threshold": 0.5,
    "ruin_reclaim_rate": 0.15,     # Moderate ruin reclaim (was 0.20)
}
...
class NorseSimulator:
    ...
    def _phase_growth(
        ...
        # v2: Moderate thresholds + higher rate = more frequent expansion
        expand_rate = p["expansion_rate"] * 0.6
        expandable = [s for s in settlements if s.alive and s.population > 1.2 and s.food > 0.7]
        if expandable:
    ...
    def _phase_winter(
        ...
        for s in settlements:
            ...
            # Food loss from winter — harsher than v1 (GT: only 26% survive)
            food_loss = severity * (0.8 + 1.0 * rng.random()) * (1.0 + s.population * 0.20)
            s.food -= food_loss

            # Population attrition in harsh winters
            if s.food < 0:
                pop_loss = min(abs(s.food) * 0.5, s.population * 0.55)
                s.population -= pop_loss
                s.food = 0.0

            # Natural population decay (disease, old age, emigration) — harsher
            s.population *= (0.95 + 0.03 * rng.random())

            # Settlement collapse check — higher thresholds
            if s.population < 0.45 or (s.food < 0.25 and rng.random() < 0.45 * severity):
                s.alive = False
                # Dead settlement outcome: GT Settlement->Empty=47%, ->Forest=23%, ->Ruin=2.5%
                adj_forest = self._count_forest_food(grid, s.y, s.x) / max(self.params["food_per_forest"], 0.1)
                roll = rng.random()
                forest_chance = 0.25 + 0.08 * min(adj_forest, 3)
    ...
    def _phase_environment(
        ...
        for ry, rx in zip(ruin_ys, ruin_xs):
            ...
            if not reclaimed:
                ...
                # Age factor: young ruins exit faster (2x v1)
                age_factor = max(0.4, 2.5 / (1.0 + ruin_age * 0.25))

                # Forest reclamation — 2x faster than v1
                if adj_forest > 0:
                    forest_prob = age_factor * forest_rate * 0.30 * min(adj_forest, 3)
                else:
                    forest_prob = age_factor * forest_rate * 0.15

                # Plains fade — secondary path
                empty_prob = age_factor * 0.025
    ...
    def run_monte_carlo(
        ...
        # Floor at 0.0005 — let near-impossible stay near-zero
