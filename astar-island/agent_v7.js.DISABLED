 ...
 * 2. Survival: compute from observed grid, not metadata (which was always 100%)
 * 3. KT threshold: n>=1 with adaptive alpha (was n>=3)
 * 4. Port suppression BEFORE KT blend (was after)
 * 5. Smarter query allocation: concentrate on fewer viewports for strong KT
 * 6. SIM_DELAY 350ms (was 280ms) to avoid rate limits
 */
...
const CLIP = [0.1, 10.0];
const SIM_DELAY = 350;   // Fix 6: was 280, ~3 req/s (limit is 5)
const SUB_DELAY = 550;
const POLL = 30000;
...
function predict(ig, H, W, pre, shift, counts, lookup) {
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    ...
    // Fix 3: Per-cell KT with adaptive alpha (n>=1)
    let p;
    const nObs = counts ? counts[y][x].reduce((a,b) => a+b, 0) : 0;
    if (nObs >= 1) {
      // Adaptive alpha: less trust with fewer observations
      const alpha = nObs >= 3 ? 0.5 : nObs === 2 ? 0.35 : 0.25;
      const ktAlpha = alpha * NC; // scale by number of categories
      p = new Array(NC);
    ...
    // Floor + normalize (port already suppressed above)
    if (!co) p[2] = FL; // reinforce after KT blend too
    let s = 0;
...
// ── Round matching — Fix 2: compute survival from grid, not metadata ────────
function computeSurvivalFromGrid(ig, observedGrid, vpX, vpY, vpH, vpW, H, W) {
  let totalSett = 0, deadSett = 0;
  for (let gy = 0; gy < vpH; gy++) for (let gx = 0; gx < vpW; gx++) {
    const ay = vpY + gy, ax = vpX + gx;
    if (ay >= H || ax >= W) continue;
    const initial = ig[ay][ax];
    if (initial === 1 || initial === 2) {
      totalSett++;
      const observed = observedGrid[gy][gx];
      // Settlement died if it's now wilderness(0/10/11), forest(3/4), or ruin(5)
      if (observed !== 1 && observed !== 2) deadSett++;
    }
  }
  return totalSett > 0 ? 1 - deadSett / totalSett : 1;
}

function matchRound(survivalRates, portRates) {
  if (Object.keys(ROUND_PROFILES).length === 0) return null;

  // Average survival across all observations
  const avgSurvival = survivalRates.length > 0
    ? survivalRates.reduce((a,b) => a+b, 0) / survivalRates.length : 1;
  const avgPort = portRates.length > 0
    ? portRates.reduce((a,b) => a+b, 0) / portRates.length : 0;

  let bestRound = null, bestDist = Infinity;
  for (const [r, profile] of Object.entries(ROUND_PROFILES)) {
    if (!profile['1']) continue;
    const histSurvival = profile['1'][1] + (profile['1'][2] || 0);
    const dist = Math.abs(avgSurvival - histSurvival) * 3 + Math.abs(avgPort - (profile['1'][2] || 0));
...
async function processRound(round) {
  ...
  // Fix 5: Plan viewports — 1 per seed (best coverage), then repeat
  const viewports = [];
  for (let si = 0; si < detail.seeds_count; si++) {
    const s = pres[si].setts;
    if (s.length === 0) continue;
    // Find single best viewport covering most settlements
    ...
    for (const st of s) {
      ...
      if (count > bestCount) { bestCount = count; bestVP = { si, x: vx, y: vy, covered: count }; }
    }
    if (bestVP) viewports.push(bestVP);
  }
  log(`Viewports: ${viewports.length} (1 per seed, covering settlements)`);

  // ── PHASE 2+3: Explore & Exploit ──────────────────────────────────────
  const obsTrans = {};
  ...
  // Keep querying until budget exhausted
  while (queriesUsed < queriesMax) {
    ...
    try {
      ...
      // Track port rate from grid observations
      let portCells = 0, totalSettObs = 0;
      for (let gy = 0; gy < result.grid.length; gy++)
        for (let gx = 0; gx < result.grid[gy].length; gx++) {
          const ay = result.viewport.y + gy, ax = result.viewport.x + gx;
          if (ay < H && ax < W && (ig[ay][ax] === 1 || ig[ay][ax] === 2)) {
            totalSettObs++;
            if (result.grid[gy][gx] === 2) portCells++;
          }
        }
      if (totalSettObs > 0) portRates.push(portCells / totalSettObs);

      qCount++;

      // Fix 1: Update budget from response if available
      if (result.queries_used !== undefined) {
      ...
      if (qCount === 5 || qCount === 10 || qCount === 20 || qCount === 30 || qCount === 40) {
        ...
        const matched = matchRound(survivalRates, portRates);
        ...
        log(`Phase ${qCount}q: survival=${avgSurv}%, match=R${matched||'?'}, budget=${queriesMax - queriesUsed} left`);
        await submitAll(round.id, detail, H, W, pres, shift,
          `Phase (${qCount}q, match=R${matched||'?'})`, cellCounts, roundLookup);
      }

    } catch (e) {
      if (e.message === '429') {
        await sleep(2000);
        // Re-check budget on rate limit
        try {
          budget = await api('GET', '/budget');
          queriesUsed = budget.queries_used || queriesUsed;
          if (queriesUsed >= (budget.queries_max || queriesMax)) {
            log(`Budget exhausted after rate limit check (${queriesUsed}/${queriesMax})`);
            break;
          }
        } catch {}
        continue;
      }
      if (e.message.includes('budget') || e.message.includes('exceeded')) {
  ...
  if (qCount > 0) {
    ...
    const avgSurv = survivalRates.length > 0
      ? (survivalRates.reduce((a,b) => a+b, 0) / survivalRates.length * 100).toFixed(0) : '?';
    log(`Final: ${qCount}q, survival=${avgSurv}%, match=R${matched||'?'}`);
    await submitAll(round.id, detail, H, W, pres, shift,
      `Final (${qCount}q, R${matched||'?'})`, cellCounts, roundLookup);
  }

  log(`Done. ${qCount} queries used, ${((Date.now()-t0)/1000).toFixed(0)}s`);
}
